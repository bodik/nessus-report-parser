...
class ReportItem(UserDict):
    ...
    def from_etree(cls, elem):
        ...
        properties = {
            ...
            'cve': [process_text_element(el) for el in elem.findall('cve')],
            'bid': [process_text_element(el) for el in elem.findall('bid')],
            'xref': [process_text_element(el) for el in elem.findall('xref')],
